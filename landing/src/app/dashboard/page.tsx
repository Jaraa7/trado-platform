...
import {
  ...
  Menu, X, Bell, Check, Lock, ChevronDown, ArrowUpRight,
  ArrowDownRight, Clock, Plus, Target, Search
} from "lucide-react";

// ─── Tier config ──────────────────────────────────────────────────
const TIER = "pro"; // في الإنتاج يأتي من JWT

const TIER_LIMITS: Record<string, {
  signals: number; exchanges: number; pairs: number;
  strategies: string[]; markets: string[]; features: string[];
}> = {
  trial: {
    signals: 3, exchanges: 1, pairs: 5,
    strategies: ["trend_following"],
    markets: ["spot"],
    features: ["signals"],
  },
  micro: {
    signals: 5, exchanges: 1, pairs: 10,
    strategies: ["trend_following", "breakout"],
    markets: ["spot"],
    features: ["signals", "alerts"],
  },
  starter: {
    signals: 15, exchanges: 2, pairs: 20,
    strategies: ["trend_following", "breakout", "mean_reversion"],
    markets: ["spot", "futures"],
    features: ["signals", "alerts", "whale_tracker", "opportunity_hunter"],
  },
  pro: {
    signals: 50, exchanges: 3, pairs: 50,
    strategies: ["trend_following", "breakout", "mean_reversion", "scalping", "custom"],
    markets: ["spot", "futures", "perpetuals"],
    features: ["signals", "alerts", "whale_tracker", "opportunity_hunter", "backtesting", "strategy_builder"],
  },
  elite: {
    signals: 150, exchanges: 5, pairs: -1,
    strategies: ["trend_following", "breakout", "mean_reversion", "scalping", "custom", "multi_strategy"],
    markets: ["spot", "futures", "perpetuals", "options"],
    features: ["signals", "alerts", "whale_tracker", "opportunity_hunter", "backtesting", "strategy_builder", "api", "tax"],
  },
  whale: {
    signals: -1, exchanges: 10, pairs: -1,
    ...
    features: ["signals", "alerts", "whale_tracker", "opportunity_hunter", "backtesting", "strategy_builder", "api", "tax", "dedicated_manager", "early_signals"],
  },
  institutional: {
    signals: -1, exchanges: -1, pairs: -1,
    strategies: ["trend_following", "breakout", "mean_reversion", "scalping", "custom", "multi_strategy", "institutional"],
    markets: ["spot", "futures", "perpetuals", "options", "defi", "cfd"],
    features: ["signals", "alerts", "whale_tracker", "opportunity_hunter", "backtesting", "strategy_builder", "api", "tax", "dedicated_manager", "early_signals", "white_label", "sub_accounts"],
  },
  founder: {
    signals: -1, exchanges: -1, pairs: -1,
    strategies: ["trend_following", "breakout", "mean_reversion", "scalping", "custom", "multi_strategy", "institutional"],
...
const MARKETS = [
  { id:"spot",       name:"Spot",       desc:"Buy/sell assets directly"      },
  { id:"futures",    name:"Futures",    desc:"Time-limited derivative contracts" },
  { id:"perpetuals", name:"Perpetuals", desc:"No-expiry futures contracts"   },
  { id:"options",    name:"Options",    desc:"Rights to buy/sell at a price" },
  { id:"defi",       name:"DeFi",       desc:"Decentralized exchange tokens" },
  { id:"cfd",        name:"CFD",        desc:"Contract for difference"       },
];

const TOP_PAIRS = [
  "BTC/USDT","ETH/USDT","SOL/USDT","BNB/USDT","XRP/USDT",
  "AVAX/USDT","DOGE/USDT","ADA/USDT","DOT/USDT","MATIC/USDT",
...
function hasExchange(n: number) { return TIER_LIMITS[TIER].exchanges === -1 || n <= TIER_LIMITS[TIER].exchanges; }

function LockedBadge() {
  return (
    <span style={{display:"inline-flex",alignItems:"center",gap:4,padding:"3px 8px",borderRadius:6,background:"rgba(251,191,36,0.15)",border:"1px solid rgba(251,191,36,0.25)",color:"#fbbf24",fontSize:10,fontWeight:600}}>
      <Lock size={9}/> Upgrade
    </span>
  );
}
...
export default function DashboardPage() {
  ...
  const [selExchanges, setSelExchanges] = useState<string[]>(["bybit"]);
  const [selMarket, setSelMarket]       = useState("spot");
  const [selPairs, setSelPairs]         = useState<string[]>(["BTC/USDT","ETH/USDT"]);
  ...
  const toggleExchange = (id: string) => {
    if (selExchanges.includes(id)) {
      setSelExchanges(selExchanges.filter(e => e !== id));
    } else {
      if (!hasExchange(selExchanges.length + 1)) return;
      setSelExchanges([...selExchanges, id]);
    }
  };
  ...
  const toggleStrategy = (id: string) => {
    if (!hasStrategy(id)) return;
    if (selStrategies.includes(id)) {
      setSelStrategies(selStrategies.filter(s => s !== id));
    } else {
      setSelStrategies([...selStrategies, id]);
    }
  };

  const filteredPairs = TOP_PAIRS.filter(p =>
      ...
      <aside style={{ width: sideOpen ? 220 : 64, background:"#080d1a", borderRight:"1px solid rgba(255,255,255,0.06)", display:"flex", flexDirection:"column", transition:"width .2s", flexShrink:0, position:"relative", zIndex:20 }}>
        ...
        <nav style={{ flex:1, padding:"10px 0" }}>
          {NAV.map(n => {
            ...
            return (
              <button key={n.id} onClick={() => !locked && setActive(n.id)} style={{
                width:"100%", display:"flex", alignItems:"center", gap:10,
                padding:"10px 14px", border:"none",
                background: active === n.id ? "rgba(14,165,233,0.1)" : "transparent",
                color: locked ? "rgba(255,255,255,0.2)" : active === n.id ? "#0ea5e9" : "rgba(255,255,255,0.45)",
                borderLeft: active === n.id ? "2px solid #0ea5e9" : "2px solid transparent",
                cursor: locked ? "not-allowed" : "pointer", transition:"all .2s",
                fontSize:13, fontWeight: active === n.id ? 600 : 400, whiteSpace:"nowrap", overflow:"hidden"
              }}>
        ...
        <div style={{ padding:"28px" }}>
          ...
          {active === "overview" && <>
            ...
            <div style={{ ...card, padding:"22px" }}>
              <h3 style={{ fontWeight:600, fontSize:14, marginBottom:16 }}>Active Config</h3>
              <div style={{ display:"flex", flexWrap:"wrap", gap:20 }}>
                <div><p style={{ fontSize:10, color:"rgba(255,255,255,0.35)", marginBottom:6, textTransform:"uppercase" }}>Exchanges</p><div style={{ display:"flex", gap:6 }}>{selExchanges.map(e => <span key={e} style={{ padding:"4px 10px", borderRadius:20, background:"rgba(14,165,233,0.15)", color:"#0ea5e9", fontSize:11, fontWeight:600, textTransform:"capitalize" }}>{e}</span>)}</div></div>
                <div><p style={{ fontSize:10, color:"rgba(255,255,255,0.35)", marginBottom:6, textTransform:"uppercase" }}>Market</p><span style={{ padding:"4px 10px", borderRadius:20, background:"rgba(16,185,129,0.15)", color:"#10b981", fontSize:11, fontWeight:600, textTransform:"capitalize" }}>{selMarket}</span></div>
                <div><p style={{ fontSize:10, color:"rgba(255,255,255,0.35)", marginBottom:6, textTransform:"uppercase" }}>Pairs</p><div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>{selPairs.slice(0,4).map(p => <span key={p} style={{ padding:"4px 10px", borderRadius:20, background:"rgba(255,255,255,0.07)", color:"rgba(255,255,255,0.7)", fontSize:11 }}>{p}</span>)}{selPairs.length > 4 && <span style={{ padding:"4px 10px", borderRadius:20, background:"rgba(255,255,255,0.05)", color:"rgba(255,255,255,0.4)", fontSize:11 }}>+{selPairs.length-4}</span>}</div></div>
                <div><p style={{ fontSize:10, color:"rgba(255,255,255,0.35)", marginBottom:6, textTransform:"uppercase" }}>Strategies</p><div style={{ display:"flex", gap:6 }}>{selStrategies.map(s => <span key={s} style={{ padding:"4px 10px", borderRadius:20, background:"rgba(99,102,241,0.15)", color:"#818cf8", fontSize:11, textTransform:"capitalize" }}>{s.replace("_"," ")}</span>)}</div></div>
              </div>
            </div>
          </>}
          ...
          {active === "signals" && <>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
              ...
              <p style={{ fontSize:12, color:"rgba(255,255,255,0.35)" }}>{limits.signals === -1 ? "Unlimited" : `${limits.signals - 47} signals remaining`} today</p>
            </div>
          ...
          {/* ── OPPORTUNITY HUNTER ── */}
          {active === "hunter" && (
            hasFeature("opportunity_hunter") ? <>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
                <div>
                  <h2 style={{ fontSize:18, fontWeight:700, marginBottom:4 }}>🎯 Opportunity Hunter</h2>
                  <p style={{ fontSize:12, color:"rgba(255,255,255,0.45)" }}>AI scanning for high-potential setups beyond your signal queue</p>
                </div>
                  ...
                  <div key={i} style={{ ...card, padding:"20px 24px", border:"1px solid rgba(251,191,36,0.15)" }}>
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"start", marginBottom:12 }}>
                      ...
                      <div style={{ textAlign:"right" }}>
                        <p style={{ fontSize:14, fontWeight:700, color:"#10b981" }}>{o.potential}</p>
                        <p style={{ fontSize:10, color:"rgba(255,255,255,0.35)" }}>potential</p>
                      </div>
                    </div>
                    ...
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                      <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                        ...
                        <div style={{ width:80, height:6, background:"rgba(255,255,255,0.08)", borderRadius:3, overflow:"hidden" }}>
                          <div style={{ height:"100%", width:`${o.conf}%`, background: o.conf>=85?"#10b981":o.conf>=70?"#fbbf24":"#0ea5e9", borderRadius:3 }}/>
                        </div>
              ...
              <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", minHeight:400, gap:16, textAlign:"center" }}>
                ...
                <p style={{ color:"rgba(255,255,255,0.5)", maxWidth:360, lineHeight:1.7 }}>AI scans the entire market for hidden opportunities beyond your signal queue. Available from Starter plan.</p>
          ...
          {active === "setup" && (
            <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
              ...
              <div style={{ ...card, padding:"22px" }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
                  <div>
                    ...
                    <p style={{ fontSize:11, color:"rgba(255,255,255,0.35)" }}>Select up to {limits.exchanges === -1 ? "unlimited" : limits.exchanges} exchange{limits.exchanges !== 1 ? "s" : ""}</p>
                  </div>
                  <span style={{ fontSize:11, color:"rgba(255,255,255,0.35)" }}>{selExchanges.length}/{limits.exchanges === -1 ? "∞" : limits.exchanges} selected</span>
                </div>
                <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
                  {EXCHANGES.map((ex, i) => {
                    const allowed = hasExchange(i + 1) || selExchanges.includes(ex.id);
                    const selected = selExchanges.includes(ex.id);
                    return (
                      <button key={ex.id} onClick={() => allowed && toggleExchange(ex.id)} style={{
                        display:"flex", alignItems:"center", gap:10, padding:"12px 18px",
                        borderRadius:12, border: selected ? `1px solid ${ex.color}40` : "1px solid rgba(255,255,255,0.08)",
                        background: selected ? `${ex.color}18` : "rgba(255,255,255,0.03)",
                        ...
                        {!allowed && <Lock size={12} style={{ color:"#fbbf24", position:"absolute", top:6, right:6 }}/>}
                      </button>
                    );
                  })}
                </div>
              ...
              <div style={{ ...card, padding:"22px" }}>
                ...
                <p style={{ fontSize:11, color:"rgba(255,255,255,0.35)", marginBottom:16 }}>Select one market to trade</p>
                <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10 }}>
                  {MARKETS.map(m => {
                    const allowed = hasMarket(m.id);
                    const selected = selMarket === m.id;
                    return (
                      <button key={m.id} onClick={() => allowed && setSelMarket(m.id)} style={{
                        padding:"14px 16px", borderRadius:12, textAlign:"left",
                        border: selected ? "1px solid rgba(14,165,233,0.4)" : "1px solid rgba(255,255,255,0.07)",
                        background: selected ? "rgba(14,165,233,0.1)" : "rgba(255,255,255,0.02)",
                        cursor: allowed ? "pointer" : "not-allowed", opacity: allowed ? 1 : 0.35,
                        transition:"all .2s", position:"relative"
                      }}>
                        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"start", marginBottom:4 }}>
                          <span style={{ fontSize:13, fontWeight:600, color: selected ? "#0ea5e9" : "#f1f5f9" }}>{m.name}</span>
                          {!allowed && <Lock size={11} style={{ color:"#fbbf24" }}/>}
                          {selected && <Check size={13} style={{ color:"#0ea5e9" }}/>}
                        </div>
                ...
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
                  <div>
                    ...
                    <p style={{ fontSize:11, color:"rgba(255,255,255,0.35)" }}>
                      {limits.pairs === -1 ? "Unlimited pairs" : `Up to ${limits.pairs} pairs`} — {selPairs.length} selected
                    </p>
                  </div>
                  <div style={{ display:"flex", alignItems:"center", gap:8, padding:"8px 12px", background:"rgba(255,255,255,0.04)", borderRadius:9, border:"1px solid rgba(255,255,255,0.08)" }}>
                    <Search size={13} style={{ color:"rgba(255,255,255,0.35)" }}/>
                    <input value={pairSearch} onChange={e => setPairSearch(e.target.value)} placeholder="Search pairs..." style={{ background:"transparent", border:"none", outline:"none", color:"#f1f5f9", fontSize:12, width:100 }}/>
                  </div>
                </div>
                <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
                  {filteredPairs.map(p => {
                    const selected = selPairs.includes(p);
                    const canAdd = limits.pairs === -1 || selPairs.length < limits.pairs;
                    const disabled = !selected && !canAdd;
                    return (
                      <button key={p} onClick={() => !disabled && togglePair(p)} style={{
                        padding:"6px 14px", borderRadius:20, fontSize:12, fontWeight:600,
                        border: selected ? "1px solid rgba(16,185,129,0.5)" : "1px solid rgba(255,255,255,0.08)",
                        background: selected ? "rgba(16,185,129,0.15)" : "rgba(255,255,255,0.03)",
                        color: selected ? "#10b981" : disabled ? "rgba(255,255,255,0.2)" : "rgba(255,255,255,0.6)",
                        cursor: disabled ? "not-allowed" : "pointer", transition:"all .15s"
                      }}>{p}</button>
                    );
                  })}
                </div>
                {limits.pairs !== -1 && selPairs.length >= limits.pairs && (
                  <div style={{ marginTop:12, padding:"10px 14px", background:"rgba(251,191,36,0.08)", border:"1px solid rgba(251,191,36,0.2)", borderRadius:9, display:"flex", alignItems:"center", gap:8 }}>
                    ...
                    <p style={{ fontSize:12, color:"#fbbf24" }}>Pair limit reached. <a href="/checkout?tier=pro" style={{ color:"#fbbf24", textDecoration:"underline" }}>Upgrade for more</a></p>
                  </div>
                )}
              </div>
              ...
              <div style={{ ...card, padding:"22px" }}>
                ...
                <p style={{ fontSize:11, color:"rgba(255,255,255,0.35)", marginBottom:16 }}>Choose one or more strategies for your AI to follow</p>
